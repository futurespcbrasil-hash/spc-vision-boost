...
Deno.serve(async (req) => {
  ...
  console.log('ryze webhook', payload?.event || payload?.type, JSON.stringify(payload).slice(0, 400));
  ...
  try {
    ...
    // Instance state
    if (event.includes('instance') || event.includes('state')) {
    ...
    if (event.includes('message') || data?.key || data?.messages) {
      const msgs = Array.isArray(data.messages) ? data.messages : [data];
      for (const m of msgs) {
        if (!m || (!m.key && !m.remoteJid)) continue;
        ...
        if (!chatId) {
          const ins = await admin.from('whatsapp_chats').insert({
            ...
            contact_number: number, contact_name: m.pushName || number,
            is_group: isGroup,
